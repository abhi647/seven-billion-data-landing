    ...
    <section id="case-studies" className="py-24 bg-background">
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          <p className="text-teal text-sm font-medium uppercase tracking-widest mb-3">Proven Results</p>
        ...
        <div className="space-y-8">
          {cases.map((c, i) => (
            <div
              key={i}
              className="bg-gradient-card border border-navy-border rounded-2xl p-8 card-glow hover:border-teal/30 transition-all"
            >
              <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
                <div className="flex items-center gap-3">
                  <span className="text-xs font-medium px-3 py-1 rounded-full bg-teal/10 text-teal border border-teal/20">
                    Case Study #{i + 1}
                  </span>
              ...
              <div className="grid md:grid-cols-2 gap-8 mb-6">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-sm font-semibold text-red-400 uppercase tracking-wider">Before</h4>
                  </div>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h4 className="text-sm font-semibold text-teal uppercase tracking-wider">After</h4>
                  </div>
                  <ul className="space-y-3">
                    {c.after.map((a, j) => (
                      <li key={j} className="flex items-start gap-2.5">
                        <span className="mt-2 h-1.5 w-1.5 rounded-full bg-teal/60 shrink-0" />
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 pt-6 border-t border-navy-border">
                <div className="flex items-center gap-2">
                  <div className="h-2 w-2 rounded-full bg-teal animate-pulse" />
                  <span className="text-sm font-medium text-teal">{c.metric}</span>
                </div>
                <Link
                  to="/apply"
                  className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-teal/10 border border-teal/30 text-teal text-sm font-semibold hover:bg-teal hover:text-navy-deep transition-all"
                >
                  {c.cta}
                  <ArrowRight size={14} />
                </Link>
